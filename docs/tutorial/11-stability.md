...
if (content.length > 100_000) {
  const filePath = `~/.blino/overflow/${sessionId}/${toolUseId}.txt`
...
// 磁盘归档（冷备）
await writeFile(`~/.blino/snapshots/${sessionId}/${safeFileName}.snapshot`, content)
```
...
```
~/.blino/projects/<hash>/<sessionId>.jsonl
```
