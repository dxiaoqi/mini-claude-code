...
| 区域 | 现状 |
|------|------|
| 用户/项目配置 | `~/.mini-claude/settings.json`、项目下 `.mini-claude/settings.json`、`.mini-claude/settings.local.json`；合并逻辑见 `src/utils/config.ts` |
...
**命名说明**：产品对外可用「blino」；仓库内历史路径为 `.mini-claude`。新增长策略层时，在实现阶段二选一或并存：**(A)** 新增 `.blino/workflow.json`（或同含义单文件）作为**策略专用**；**(B)** 在 `.mini-claude/settings.json` 中增加 `policy` 嵌套字对象。本规划不强行改名现有目录，避免一次 PR 全仓替换路径。
...
│  建议落点: .blino/workflow.json（或 .mini-claude/policy.json）│
└──────────────────────┬──────────────────────────────────┘
