...
export function handleError(error: unknown, context: ErrorContext = {}): never {
  ...
  // Convert to BlinoError if needed
  const normalizedError: BlinoError = error instanceof BlinoError
    ? error
    : error instanceof Error
      ? (() => {
        ...
        // Default to generic BlinoError
        return new BlinoError(error.message, {
          context,
          userFriendly: false,
        })
      })()
      : new BlinoError(String(error), {
          context,
          userFriendly: false,
        })
