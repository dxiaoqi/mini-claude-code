...
program
  .name('mini-claude')
  .description('Mini Claude Code — AI coding assistant')
  .version(VERSION)
  .option('--api-key <key>', 'API key (reads from ~/.mini-claude/settings.json if not set)')
...
program.action(async (prompt: string | undefined, options: Record<string, unknown>) => {
  ...
  if (!apiKey) {
    ...
    console.error(chalk.dim('  1. ~/.mini-claude/settings.json  →  { "api": { "anthropicApiKey": "..." } }'))
  ...
  if (providerType === 'anthropic') {
    ...
    if (!baseURL) {
      ...
      console.error(chalk.dim('Set it via openaiBaseUrl in ~/.mini-claude/settings.json or OPENAI_BASE_URL env.'))
      process.exit(1)
    }
  ...
  console.log(chalk.bold.cyan('║      Mini Claude Code v' + VERSION + '        ║'))
  console.log(chalk.bold.cyan('╚══════════════════════════════════════╝'))
  ...
  for await (const userInput of adapter.getUserInput()) {
    ...
    if (userInput.startsWith('/config set ')) {
      ...
      if (key === 'model') {
        ...
        console.log(chalk.green('Saved anthropicApiKey to ~/.mini-claude/settings.json'))
      } else if (key === 'api.anthropicBaseUrl') {
        await saveApiConfig('user', cwd, { anthropicBaseUrl: value })
        console.log(chalk.green('Saved anthropicBaseUrl to ~/.mini-claude/settings.json'))
      } else if (key === 'api.openaiApiKey') {
        await saveApiConfig('user', cwd, { openaiApiKey: value })
        console.log(chalk.green('Saved openaiApiKey to ~/.mini-claude/settings.json'))
      } else if (key === 'api.model') {
        await saveApiConfig('user', cwd, { model: value })
        console.log(chalk.green(`Saved default model to ~/.mini-claude/settings.json`))
      } else {
