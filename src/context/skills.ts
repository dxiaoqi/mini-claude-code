 ...
 *
 * 从用户与项目目录下的 .lumi/skills 加载 Markdown 技能文件，解析 YAML
...
/**
 * Load skills from .lumi/skills/ directories.
...
export async function loadSkills(projectRoot: string): Promise<SkillDefinition[]> {
  ...
  // User-level skills
  if (homeDir) {
    const userSkillsDir = resolve(homeDir, '.lumi', 'skills')
    const userSkills = await loadSkillsFromDir(userSkillsDir, 'user')
    skills.push(...userSkills)
  }

  // Project-level skills
  const projectSkillsDir = resolve(projectRoot, '.lumi', 'skills')
