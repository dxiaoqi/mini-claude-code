 ...
 *
 * 加载项目内（如 `.lino/skills/`）定义的 Skill，将对应工作流说明注入对话供模型遵循。
 */
import { z } from 'zod'
...
export const SkillTool: Tool<Input, Output> = {
  name: 'Skill',
  aliases: ['SkillTool'],
  description: 'Execute a project-defined skill (workflow). Skills are Markdown prompt files in .lino/skills/. Use SkillTool to discover and run them.',
    ...
    if (input.skill === 'list' || input.skill === 'help') {
      if (cachedSkills.length === 0) {
        return {
          data: {
            skillName: 'list',
            prompt: 'No skills found. Create .md files in .lino/skills/ to define skills.',
            found: false,
          },
        }
      }
